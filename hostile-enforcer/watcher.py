...
def main():
    ...
    print(f"[Hostile Enforcer] Watching. Blocked sites: {blocked_sites}")
    ...
    while True:
        ...
        if is_blocked:
            if dwell >= 60 and not punished_60s:
                ...
                send_notification("Hostile Enforcer", f"Time's up. Closing Chrome. You wasted 60s on {domain}.")
                close_chrome()
                change_wallpaper()
                punished_60s = True
            elif dwell >= 30 and not warned_30s:
                print(f"  ESCALATION: 30s — final warning")
                send_notification("Hostile Enforcer", f"Still on {domain}! Chrome WILL be closed in 30 seconds.")
                warned_30s = True
            elif dwell >= 10 and not warned_10s:
                print(f"  ESCALATION: 10s — first warning")
                send_notification("Hostile Enforcer", f"You've been on {domain} for 10 seconds. Get back to work!")
                warned_10s = True
